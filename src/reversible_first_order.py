...
class ReversibleFirstOrder:
    ...
    def __init__(
        ...
        rate_cte_forward,
        rate_cte_backward,
        reactant_initial_concentration,
        product_initial_concentration,
        time=10,
        points=100,
    ):
        """
    ...
    def reactant_time(self):
        ...
        """
        return -(
            (self.kb * self.B0 - self.kf * self.A0)
            * np.exp(-self.t * (self.kf + self.kb))
            - self.kb * (self.A0 + self.B0)
        ) / (self.kf + self.kb)
    ...
    def _plot_params(
        ...
        if not title:
            ...
            ax.set_title(
                r"Concentration vs time for Reactant $\rightleftarrows$ Product",
                fontsize=18,
            )
        else:
    ...
    def plot_reaction_time(
        ...
        self._plot_params(
            ax, time_unit, "Concentration", concentration_unit, title, title_text
        )

        handles, labels = ax.get_legend_handles_labels()
        patch_kf = mpatches.Patch(
            color="white", label=" ".join(["$k_f =$", f"{self.kf}"])
        )
        patch_kb = mpatches.Patch(
            color="white", label=" ".join(["$k_b =$", f"{self.kb}"])
        )
        patch_A0 = mpatches.Patch(
            color="white", label=" ".join(["$[A]_0 =$", f"{self.A0}"])
        )
        patch_BO = mpatches.Patch(
            color="white", label=" ".join(["$[B]_0 =$", f"{self.B0}"])
        )
    ...
    def plot_reaction_quotient_time(
        self,
        size=(10, 8),
        ax=None,
        time_unit="s",
